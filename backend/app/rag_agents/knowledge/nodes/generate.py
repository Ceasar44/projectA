...
def prepare_generation_context(state: KnowledgeAgentState, config=None) -> Dict[str, Any]:
    ...
    if collection:
        try:
            ...
            from app.rag.db.session import run_with_session_sync

            kb = run_with_session_sync(
                lambda session: get_kb_repository(session).get_by_name(collection)
            )
    ...
    if is_image_mode:
        try:
            from app.rag.db import get_chunk_image_repository
            from app.rag.db.session import run_with_session_sync
            ...
            if chunk_ids:
                img_records = run_with_session_sync(
                    lambda session: get_chunk_image_repository(session).get_by_chunk_ids(chunk_ids)
                )
    ...
    if is_image_mode:
        try:
            ...
            if chunk_ids_for_img:
                img_records = run_with_session_sync(
                    lambda session: get_chunk_image_repository(session).get_by_chunk_ids(
                        list(set(chunk_ids_for_img))
                    )
                )
                for r in img_records:
                    ph = r.get("placeholder", "")
