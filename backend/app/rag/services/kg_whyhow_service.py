...
class KGRetrievalService:
    ...
    def _enrich_from_pg(
        ...
        try:
            from app.rag.db import get_chunk_repository
            from app.rag.db.session import run_with_session_sync

            pg_rows = run_with_session_sync(
                lambda session: get_chunk_repository(session).get_by_ids_with_file_names(
                    list(whyhow_chunks.keys())
                )
            )
