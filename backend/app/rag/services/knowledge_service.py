...
from app.rag.core.config import settings, SUPPORTED_MODELS
from app.rag.core.exceptions import ValidationError, ExternalServiceError
...
async def _persist_conversation_messages(
    ...
    try:
        ...
        from app.rag.db.session import run_with_session

        async def _write(session):
            conv_repo = get_conversation_repository(session)
            if not await conv_repo.get_session(session_id):
                return
            ...
            await conv_repo.touch_session(session_id)
            await session.commit()

        await run_with_session(_write)
...
async def _load_kb_retrieval(collection: Optional[str]):
    ...
    try:
        ...
        async def _load(session):
            return await get_kb_repository(session).get_by_name(collection)

        kb = await run_with_session(_load)
        if kb:
...
async def invoke_knowledge_qa(
    ...
    from app.rag_agents.knowledge import get_knowledge_agent, create_initial_state, RAGConfig
...
async def stream_knowledge_qa_sse(
    ...
    from app.rag_agents.knowledge import get_knowledge_stream_prep_agent, create_initial_state, RAGConfig
    from app.rag_agents.knowledge.nodes.generate import (
        prepare_generation_context,
        _sanitize_image_placeholders,
        build_sources_from_reranked,
    )
    from app.rag_agents.knowledge.openai_stream import iter_openai_text_deltas
