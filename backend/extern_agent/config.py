...
# They merely document the expected format 鈥?put real values in config.json.
available_setting = {
    ...
    "clear_memory_commands": ["#娓呴櫎璁板繂"],  # session-reset command; must start with #
    # channel config
...
def load_config():
    ...
    # same key twice (e.g. two `"tools"` blocks) 鈥?json.loads would
...
def _merge_duplicate_keys(pairs):
    ...
    for key, val in pairs:
        ...
        if isinstance(prev, dict) and isinstance(val, dict):
            ...
            out[key] = val
    if duplicates:
        # logger may not be wired yet 鈥?fall back to print so we never lose the warning.
