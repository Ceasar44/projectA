...
const TONE_OPTIONS = [
  { value: "friendly", label: "友好", desc: "温暖、亲切、容易接近" },
  { value: "professional", label: "专业", desc: "清晰、稳重、商务化" },
  { value: "formal", label: "正式", desc: "礼貌、严谨、有分寸" },
  { value: "technical", label: "技术型", desc: "准确、详细、重视细节" },
];
...
const PROVIDER_OPTIONS = [
  ...
  { value: "ollama", label: "Ollama (本地)", models: ["llama3", "mistral", "codellama", "phi3"] },
];
...
export default function SetupPage() {
  ...
  const [welcomeMessage, setWelcomeMessage] = useState("您好！今天有什么可以帮您？");
  ...
  async function handleNext() {
    ...
    try {
      if (step === 0) {
        ...
        if (!res.ok) {
          const data = await res.json();
          setError(data.error || "保存 AI 配置失败。");
          setLoading(false);
          return;
        }
    ...
    <div className="bg-owly-surface rounded-2xl shadow-lg border border-owly-border overflow-hidden">
      ...
      <div className="bg-owly-primary-50 border-b border-owly-border px-8 pt-6 pb-4">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h1 className="text-lg font-bold text-owly-text">设置 Owly</h1>
