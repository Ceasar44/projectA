...
export default function ConversationsPage() {
  ...
  const normalizeConversation = useCallback((conversation: ConversationData) => {
    const rawTags: RawTagData[] = Array.isArray(conversation.tags)
      ? conversation.tags
      : [];
    return {
      ...conversation,
      tags: rawTags.map((item): TagData => {
        if (item.tag) {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      setConversations((data.data || []).map(normalizeConversation));
    } catch (error) {
      console.error("Failed to fetch conversations:", error);
      setFetchError("加载会话失败，请尝试刷新页面。");
    } finally {
      setLoading(false);
    }
  }, [channelFilter, statusFilter, searchQuery, normalizeConversation]);
