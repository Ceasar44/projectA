...
const PRIORITIES = [
  { value: 0, label: "普通", icon: Minus, className: "bg-gray-100 text-gray-600" },
  { value: 1, label: "中等", icon: ArrowUp, className: "bg-yellow-100 text-yellow-700" },
  { value: 2, label: "高", icon: ArrowUp, className: "bg-orange-100 text-orange-700" },
  { value: 3, label: "关键", icon: Star, className: "bg-red-100 text-red-700" },
];
...
export default function KnowledgeBasePage() {
  ...
  const fetchEntries = useCallback(async (categoryId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        const data: EntryListResponse = await res.json();
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFeedback({
          ...
          message:
            payload?.error ||
            payload?.detail ||
            `Failed to delete ${deleteTarget.type === "category" ? "category" : "entry"}.`,
        });
      }
    } catch (err) {
      ...
      setFeedback({
        type: "error",
        message: `Failed to delete ${deleteTarget.type === "category" ? "category" : "entry"}.`,
      });
    } finally {
      setDeleting(false);
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="w-80 flex-shrink-0 border-r border-owly-border bg-owly-surface flex flex-col">
          <div className="px-4 py-3 border-b border-owly-border flex items-center justify-between">
            <h3 className="text-sm font-semibold text-owly-text">分类</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingCategories ? (
              ...
              <div className="px-4 py-12 text-center">
                ...
                <p className="text-sm font-medium text-owly-text-light">暂无分类</p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="填写 AI 回复客户时会使用的知识内容..."
                  rows={8}
          ...
          <div className="relative bg-owly-surface rounded-xl shadow-xl border border-owly-border w-full max-w-sm mx-4">
            <div className="p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-owly-text">
                  删除{deleteTarget.type === "category" ? "分类" : "条目"}
                </h3>
              </div>
