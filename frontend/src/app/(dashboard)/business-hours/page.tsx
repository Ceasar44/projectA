...
export default function BusinessHoursPage() {
  ...
  const currentStatus = useMemo(() => {
    if (!config.enabled) return { open: false, label: "客服支持已停用" };
    ...
    try {
      ...
      if (!daySchedule) return { open: false, label: "今天休息" };

      const times = parseTime(daySchedule);
      if (!times) return { open: false, label: "今天休息" };
      ...
      return {
        ...
        label: isOpen
          ? `当前营业中（至 ${times.end}）`
          : `已休息（${times.start} 开始营业）`,
      };
    } catch {
      return { open: false, label: "无法判断当前状态" };
    }
  }, [config]);
                  ...
                  >
                    ...
                    {isOpen && times ? (
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <span className="text-xs text-owly-text-light">至</span>
                        <select
                          value={times.end}
                          onChange={(e) =>
